...
def floating_note():
    ...
    <div class="floating-note">
      ...
      <a href="mailto:ambinintsoa.uat.ead2@gmail.com" title="Mail">
        <img src="https://img.icons8.com/?size=100&id=86875&format=png&color=000000" alt="Mail">
      </a>
      <a href="https://github.com/ramanambonona" target="_blank" rel="noopener" title="GitHub">
        <img src="https://img.icons8.com/?size=100&id=3tC9EQumUAuq&format=png&color=000000" alt="GitHub">
      </a>
      <a href="https://www.linkedin.com/in/ambinintsoa-ramanambonona" target="_blank" rel="noopener" title="LinkedIn">
...
def forecast_prophet(df, col, periods, changepoint=0.05, seasonality=10.0):
    ...
    try:
        ...
        m.fit(pdf)
        fut = m.make_future_dataframe(periods=periods, freq="M")
...
def build_module(df: pd.DataFrame, module_title: str):
    ...
    with tab2:
        ...
        if "Date" not in df.columns:
            ...
            if vars_sel:
                fig = px.line(
                    df, x="Date", y=vars_sel,
                    labels={"value":"Valeur","variable":"Variable"},
                    height=520, title=f"Évolution — {module_title}"
                )
    ...
    with tab4:
        ...
        models = [
            "NAIVE","AR(p)","ARIMA","VAR","ARDL",
            "Régression Linéaire","Random Forest","MLP",
            "Exponential Smoothing (ETS)","SNaive","Auto (min MAPE)"
        ]
        if PROPHET_OK:
            models.insert(5, "Prophet")
